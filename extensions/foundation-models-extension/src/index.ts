 ...
 * Intelligence) as a Mita AI engine. The model runs fully locally — no internet
 * connection or external API key is required.
 *
 * Architecture:
 *   Mita extension (TypeScript) → Tauri plugin (Rust / fm-rs) → FoundationModels.framework
 *
...
export default class FoundationModelsExtension extends AIEngine {
  ...
  override async delete(_modelId: string): Promise<void> {
    throw new Error(
      'Apple Foundation Models are part of the operating system and cannot be deleted from Mita.'
    )
  }

  override async update(_modelId: string, _model: Partial<modelInfo>): Promise<void> {
    throw new Error(
      'Apple Foundation Models are managed by the OS and cannot be updated from Mita.'
    )
  }
