...
#[tauri::command]
pub fn get_mita_extensions_path<R: Runtime>(app_handle: tauri::AppHandle<R>) -> PathBuf {
    get_mita_data_folder_path(app_handle).join("extensions")
}

#[tauri::command]
pub fn get_jan_extensions_path<R: Runtime>(app_handle: tauri::AppHandle<R>) -> PathBuf {
    get_mita_extensions_path(app_handle)
}
...
pub fn get_active_extensions<R: Runtime>(app: AppHandle<R>) -> Vec<serde_json::Value> {
    ...
    {
        let mut path = get_mita_extensions_path(app);
        path.push("extensions.json");
        log::info!("get mita extensions, path: {path:?}");
