...
pub async fn run_mcp_commands<R: Runtime>(
    ...
    let mut mcp_servers: serde_json::Value = serde_json::from_str(&config_content)
    ...
    {
        if normalize_browser_mcp_server_key(server_map) {
            std::fs::write(
                &config_path,
    ...
    let servers_to_stop: Vec<(String, RunningServiceEnum, Option<u16>)> = {
        ...
        for key in keys {
            if let Some(service) = servers_map.remove(&key) {
                let port = if is_browser_mcp_name(&key) {
