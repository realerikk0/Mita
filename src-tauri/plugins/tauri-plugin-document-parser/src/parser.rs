use crate::{DocumentParserError, MAX_DOCUMENT_FILE_SIZE};
...
pub fn parse_pdf(file_path: &str) -> Result<String, DocumentParserError> {
    let metadata = fs::metadata(file_path)?;
    if metadata.len() > MAX_DOCUMENT_FILE_SIZE {
        return Err(DocumentParserError::ParseError(
    ...
    let text = match catch_unwind(AssertUnwindSafe(|| {
        ...
        Ok(Ok(t)) => t,
        Ok(Err(e)) => {
            return Err(DocumentParserError::ParseError(format!(
                "PDF parse error: {}",
                e
            )))
        }
...
pub fn parse_text(file_path: &str) -> Result<String, DocumentParserError> {
    read_text_auto(file_path)
}

pub fn parse_document(file_path: &str, file_type: &str) -> Result<String, DocumentParserError> {
        ...
        other => {
            ...
            match infer::get_from_path(file_path) {
                Ok(Some(k)) => {
                    ...
                    Err(DocumentParserError::UnsupportedFileType(other.to_string()))
                }
                _ => {
...
fn parse_docx(file_path: &str) -> Result<String, DocumentParserError> {
    ...
    let mut zip =
        ZipArchive::new(file).map_err(|e| DocumentParserError::ParseError(e.to_string()))?;
    ...
    doc_xml
        ...
        .map_err(|e| DocumentParserError::ParseError(e.to_string()))?;
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            ...
            Ok(Event::Eof) => break,
            Err(e) => return Err(DocumentParserError::ParseError(e.to_string())),
            _ => {}
        }
    }
...
fn parse_csv(file_path: &str) -> Result<String, DocumentParserError> {
    let metadata = fs::metadata(file_path)?;
    if metadata.len() > MAX_DOCUMENT_FILE_SIZE {
        return Err(DocumentParserError::ParseError(
            "File too large (max 20MB)".to_string(),
    ...
    for rec in rdr.records() {
        let rec = rec.map_err(|e| DocumentParserError::ParseError(e.to_string()))?;
...
fn parse_spreadsheet(file_path: &str) -> Result<String, DocumentParserError> {
    let metadata = fs::metadata(file_path)?;
    if metadata.len() > MAX_DOCUMENT_FILE_SIZE {
        return Err(DocumentParserError::ParseError(
            "File too large (max 20MB)".to_string(),
...
fn parse_pptx(file_path: &str) -> Result<String, DocumentParserError> {
    let metadata = std::fs::metadata(file_path)?;
    if metadata.len() > MAX_DOCUMENT_FILE_SIZE {
        return Err(DocumentParserError::ParseError(
            "File too large (max 20MB)".to_string(),
...
fn parse_html(file_path: &str) -> Result<String, DocumentParserError> {
    let html = read_text_auto(file_path)?;
    // 80-column wrap default
    Ok(html2text::from_read(Cursor::new(html), 80))
}

fn read_text_auto(file_path: &str) -> Result<String, DocumentParserError> {
    let metadata = fs::metadata(file_path)?;
    if metadata.len() > MAX_DOCUMENT_FILE_SIZE {
        return Err(DocumentParserError::ParseError(
            "File too large (max 20MB)".to_string(),
...
mod tests {
    ...
    impl TestFile {
        fn new(extension: &str) -> Self {
            ...
            Self(std::env::temp_dir().join(format!(
                "biyan-document-parser-{}-{nonce}.{extension}",
                std::process::id()
            )))
        }
    ...
    #[test]
    fn parses_complete_text_without_truncating() {
        let file = TestFile::new("md");
        let expected = "# Biyan\n\nComplete document body.\nLast line.";
        fs::write(&file.0, expected).expect("fixture should be writable");

        assert_eq!(
            parse_document(file.path(), "text/markdown").unwrap(),
            expected
        );
    }

    #[test]
    fn supports_source_code_as_plain_text() {
        let file = TestFile::new("rs");
        let expected = "fn main() { println!(\"hello\"); }";
        fs::write(&file.0, expected).expect("fixture should be writable");

        assert_eq!(parse_document(file.path(), "rs").unwrap(), expected);
    }

    #[test]
    fn rejects_every_file_larger_than_twenty_megabytes() {
        ...
        handle
            .set_len(MAX_DOCUMENT_FILE_SIZE + 1)
            .expect("sparse fixture should be resizable");

        let error = parse_document(file.path(), "txt").unwrap_err();
        assert!(error.to_string().contains("max 20MB"));
    }

    #[test]
    fn rejects_unsupported_binary_files() {
        let file = TestFile::new("png");
        fs::write(&file.0, b"\x89PNG\r\n\x1a\n").expect("fixture should be writable");

        assert!(matches!(
            parse_document(file.path(), "png"),
            Err(DocumentParserError::UnsupportedFileType(kind)) if kind == "image/png"
        ));
    }
}
