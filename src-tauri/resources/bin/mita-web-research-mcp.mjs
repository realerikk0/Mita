...
const PROFILE_DIR =
  process.env.MITA_WEB_RESEARCH_PROFILE_DIR ||
  `${process.env.HOME || process.cwd()}/.mita-web-research`
const HEADLESS = process.env.MITA_WEB_RESEARCH_HEADLESS !== 'false'
...
async function ensureBrowserPage() {
  ...
  let playwright
  try {
    const modulePath = process.env.MITA_WEB_RESEARCH_PLAYWRIGHT_MODULE
