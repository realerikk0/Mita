...
| --- | --- | --- | --- |
| Complete | `v0.6.648` | `33e8c5b03278b2b91318a553eb3b699b19c6ad1e` | `C/3` |
...
The `v0.6.646` and `v0.6.647` tags, Draft releases, uploaded assets, and failed
direct qualifications are immutable blocked-before-publication evidence.
Neither candidate was published or promoted; preserve them exactly, do not
retag or reuse them, and do not recut A/B/C. `v0.6.648` is the single higher
...
There is no A or B dispatch. One accepted `v0.6.648/C/3` candidate is the only
...
- Windows and macOS: `0.6.608 → 0.6.648` as manual-installer compatibility
  only, with the retired endpoint and old public-key generation kept explicit;
- Windows and macOS: `0.6.611 → 0.6.648` as the lowest public stable
  automatic-updater path;
- Windows and macOS: `0.6.633 → 0.6.648`;
- Linux: fresh `0.6.648`, under the exact no-current-Linux-artifact exception;
...
exact reviewed `0.6.648` manifest SHA-256 and four-platform set. It remains
...
`0.6.643 → 0.6.648`, `0.6.644 → 0.6.648`, and
`0.6.645 → 0.6.648`. It also replaces both legacy manifests byte-identically,
so clients `0.6.609–0.6.633` receive `0.6.648` directly. `0.6.609` and
...
old-private-key proof. The `0.6.608 → 0.6.648` lanes prove manual installation
compatibility only.
