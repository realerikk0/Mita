...
const artifactRoot = artifactRootIndex >= 0 ? path.resolve(process.argv[artifactRootIndex + 1]) : null
const failures = []
...
function walk(root, callback) {
  ...
  for (const entry of fs.readdirSync(root, { withFileTypes: true })) {
    if (entry.isDirectory() && ['node_modules', 'target', '.next', '.git', '.yarn'].includes(entry.name)) continue
...
if (fs.existsSync(path.join(repoRoot, '.github/workflows/publish-npm-core.yml'))) {
  fail('public npm workflow still exists: .github/workflows/publish-npm-core.yml')
}
...
walk(workflowRoot, (file) => {
  ...
  if (/\b(?:npm publish|yarn npm publish|npm publish)\b/.test(source)) {
    fail(`public npm publish command is forbidden: ${path.relative(repoRoot, file)}`)
  }
})
...
if (!desktopRelease.includes('BIYAN_DATA_SCHEMA: ${{ needs.preflight.outputs.data_schema }}')) {
  fail('desktop release does not compile the attested Biyan data schema into candidates')
}
...
for (const message of validateFlatpakMetadata(flatpakManifest, flatpakMetainfo)) fail(message)
...
for (const manifest of packageManifests) {
  ...
  if (packageJson.private !== true) fail(`${path.relative(repoRoot, manifest)} must be private`)
}
...
const isAllowlistedLegacyPath = (relative) => (
  /(?:^|[/\\])(?:__tests__|tests?|fixtures|legacy_migrations)(?:[/\\]|\.)/.test(relative)
  || /(?:^|[/\\])(?:LICENSE|NOTICE)(?:\.[^/\\]+)?$/.test(relative)
)
for (const relativeRoot of ['src-tauri/src', 'src-tauri/plugins', 'web-app/src', 'core', 'extensions']) {
  walk(path.join(repoRoot, relativeRoot), (file) => {
    ...
    for (const pattern of retiredActivePatterns) {
      if (pattern.test(source)) fail(`retired Jan identifier or network endpoint found in active source: ${relative}`)
    }
  })
}

for (const relative of [
  'package.json', 'core/package.json', 'web-app/package.json', 'extensions/package.json',
  'src-tauri/Cargo.toml', 'src-tauri/utils/Cargo.toml', 'yarn.lock',
  'src-tauri/Cargo.lock', 'src-tauri/plugins/yarn.lock',
]) {
  const absolute = path.join(repoRoot, relative)
  if (!fs.existsSync(absolute)) continue
  const source = fs.readFileSync(absolute, 'utf8')
  for (const pattern of retiredActivePatterns.slice(3)) {
    if (pattern.test(source)) fail(`retired Jan package/extension identifier found in manifest or lockfile: ${relative}`)
  }
}
...
for (const manifest of packageManifests) {
  ...
  for (const pattern of retiredActivePatterns.slice(3)) {
    if (pattern.test(source)) fail(`retired Jan package/extension identifier found in package manifest: ${relative}`)
  }
}

for (const relative of ['src-tauri/static/openapi.json', 'docs/public/openapi/biyan.json']) {
  const absolute = path.join(repoRoot, relative)
  if (!fs.existsSync(absolute)) continue
...
for (const relative of [
  ...
  const source = read(relative)
  if (!/resources\/bin\/biyan-cli/.test(source)) fail(`${relative} must package the biyan CLI`)
  if (/resources\/bin\/(?:jan|mita)(?:-cli)?/i.test(source)) {
    fail(`${relative} still packages a retired CLI`)
  }
}
const windowsWorkflow = read('.github/workflows/template-tauri-build-windows-x64.yml')
if (!/Biyan_\$\{\{ inputs\.new_version \}\}_x64-setup\.exe/.test(windowsWorkflow)) {
  fail('Windows build template does not enforce the Biyan installer name')
}
if (!/biyan-windows-x64-\$\{\{ inputs\.new_version \}\}-(?:nsis|msi)/.test(windowsWorkflow)) {
...
function binaryContains(file, needles) {
  ...
  try {
    let position = 0
    while (true) {
      const bytesRead = fs.readSync(descriptor, buffer, 256, 1024 * 1024, position)
      if (bytesRead === 0) return null
      const chunk = Buffer.concat([carry, buffer.subarray(256, 256 + bytesRead)]).toString('latin1').toLowerCase()
...
if (artifactRoot) {
  if (!fs.existsSync(artifactRoot)) fail(`artifact root does not exist: ${artifactRoot}`)
  walk(artifactRoot, (file) => {
    const name = path.basename(file)
    if (/\b(?:jan|mita)(?:[-_.]|$)/i.test(name)) {
      fail(`release artifact uses a retired product name: ${path.relative(artifactRoot, file)}`)
    }
    if (/\.(?:tgz|zip|exe|msi|dmg|AppImage|deb|rpm)$/i.test(name) && /(?:llama|mlx|foundation|rag|vector)/i.test(name)) {
      fail(`release artifact exposes a retired local runtime: ${path.relative(artifactRoot, file)}`)
    }
    const retiredPayload = binaryContains(file, [
      'github.com/janhq/llama.cpp', 'catalog.jan.ai', 'apps.jan.ai',
      'apps-nightly.jan.ai', '@janhq/', 'jan-utils', 'llamacpp-extension',
      'rag-extension', 'vector-db-extension', 'foundation-models-extension', 'mlx-extension',
    ])
    if (retiredPayload) {
      fail(`release artifact contains retired payload "${retiredPayload}": ${path.relative(artifactRoot, file)}`)
    }
  })
}
