...
test('live promotion exposes only DIRECT_C and RECOVERY and requires exact 16 of 16', () => {
  ...
  assert.match(source, /"Biyan Upgrade Smoke"/)
  assert.match(source, /github-native-direct-qualification/)
  assert.match(source, /github-native-recovery-qualification/)
  assert.match(source, /\.sampleSize == 16/)
  assert.match(source, /\(\.results \| length\) == 16/)
  assert.match(source, /\.failedAttempts == 0/)
  assert.match(source, /--json attempt,conclusion,headSha,workflowName,url/)
  assert.match(source, /git merge-base --is-ancestor "\$candidate_source" "\$smoke_head"/)
  assert.match(source, /git merge-base --is-ancestor "\$smoke_head" refs\/remotes\/origin\/mita-main/)
  assert.match(source, /RECOVERY qualification harness drifted after the smoke run/)
  assert.match(source, /biyan-promotion-evidence-\$\{SMOKE_RUN_ID\}-\$\{smoke_attempt\}/)
  assert.match(source, /gh api --paginate --slurp/)
  assert.match(source, /\$matches \| length\) == 1/)
  assert.match(source, /\$matches\[0\]\.expired == false/)
  assert.match(source, /\$matches\[0\]\.digest \| test\("\^sha256:\[0-9a-f\]\{64\}\$"\)/)
  assert.match(source, /\.sourceVersion == \$from/)
  assert.match(source, /\.deploymentMode == "recovery"/)
  assert.match(source, /\.platforms == \["windows", "macos", "linux"\]/)
  assert.match(source, /\.scenarios == \["source-to-recovery"\]/)
  assert.match(source, /\.sampleSize == 3/)
  assert.match(source, /\.passedAttempts == 3/)
  assert.match(
    source,
