...
legacy_aliyun_url="https://static.mitapp.cn/mita/latest.json"
legacy_r2_url="https://updates.mita.so/mita/latest.json"
...
recover_terminal_open_journal() {
  ...
  local policy_index policy_existed
  ...
  probe_oss "$legacy_aliyun_key" terminal-frozen-legacy-oss
  probe_r2 "$legacy_r2_key" terminal-frozen-legacy-r2
  jq -e '.state == "exists"' \
    dist/state/probes/terminal-frozen-legacy-oss.json >/dev/null
  jq -e '.state == "exists"' \
    dist/state/probes/terminal-frozen-legacy-r2.json >/dev/null
  ossutil cp "oss://${ALIYUN_OSS_BUCKET}/${legacy_aliyun_key}" \
    dist/state/terminal-recovery/live/frozen-legacy-oss.json --force \
    --endpoint "$oss_endpoint" --region "$ALIYUN_REGION"
  aws s3 cp "s3://${CLOUDFLARE_R2_BUCKET}/${legacy_r2_key}" \
    dist/state/terminal-recovery/live/frozen-legacy-r2.json \
    --endpoint-url "$r2_endpoint"
  node scripts/updater/promotion-transaction.mjs validate-frozen-legacy \
    --current-policy dist/state/terminal-recovery/backups/policy.json \
    --next-policy dist/state/terminal-recovery/next-policy-r2.json \
    --oss-legacy dist/state/terminal-recovery/live/frozen-legacy-oss.json \
    --r2-legacy dist/state/terminal-recovery/live/frozen-legacy-r2.json \
    --output dist/state/terminal-recovery/live/frozen-legacy-invariant.json
  node scripts/updater/promotion-transaction.mjs poll-url \
    --url "$legacy_aliyun_url" \
    --expected dist/state/terminal-recovery/live/frozen-legacy-oss.json \
    --timeout-seconds 600 --interval-seconds 10 \
    --output dist/state/terminal-recovery/live/frozen-legacy-aliyun-cdn.json
  node scripts/updater/promotion-transaction.mjs poll-url \
    --url "$legacy_r2_url" \
    --expected dist/state/terminal-recovery/live/frozen-legacy-r2.json \
    --timeout-seconds 600 --interval-seconds 10 \
    --output dist/state/terminal-recovery/live/frozen-legacy-r2-cdn.json
...
current_legacy_version="$(jq -er .legacyBridgeVersion \
...
node scripts/updater/promotion-transaction.mjs validate-frozen-legacy \
  ...
  --output dist/state/frozen-legacy-invariant.json
...
rollback() {
  ...
  # Phase 2 may mutate only after every classification succeeded. The frozen
  # legacy handoff is never a recovery target; the Router policy is restored
  # with content CAS and the immutable ledger is cleaned independently.
  ...
  if [[ "$rollback_failed" -eq 0 ]]; then
    ossutil cp "oss://${ALIYUN_OSS_BUCKET}/${legacy_aliyun_key}" \
      dist/state/rollback-frozen-legacy-oss.json --force \
      --endpoint "$oss_endpoint" --region "$ALIYUN_REGION" \
      || rollback_failed=1
    aws s3 cp "s3://${CLOUDFLARE_R2_BUCKET}/${legacy_r2_key}" \
      dist/state/rollback-frozen-legacy-r2.json \
      --endpoint-url "$r2_endpoint" || rollback_failed=1
    node scripts/updater/promotion-transaction.mjs validate-frozen-legacy \
      --current-policy dist/state/snapshots/policy.json \
      --next-policy dist/state/next-policy.json \
      --oss-legacy dist/state/rollback-frozen-legacy-oss.json \
      --r2-legacy dist/state/rollback-frozen-legacy-r2.json \
      --output dist/state/rollback-frozen-legacy-invariant.json \
      || rollback_failed=1
  fi
...
# The policy CAS is deliberately last. Both retired legacy endpoints have
# already been proven byte-identical to the frozen policy digest; they are
# read-only invariants and never promotion targets. Any failure around this
# point is still covered by the same durable journal and rollback boundary.
if [[ "$policy_existed" == true ]]; then
  aws s3api get-object --bucket "$CLOUDFLARE_R2_BUCKET" \
    --key "$POLICY_KEY" --endpoint-url "$r2_endpoint" \
...
ossutil cp "oss://${ALIYUN_OSS_BUCKET}/${legacy_aliyun_key}" \
  dist/state/frozen-legacy-oss-final.json --force \
  --endpoint "$oss_endpoint" --region "$ALIYUN_REGION"
aws s3 cp "s3://${CLOUDFLARE_R2_BUCKET}/${legacy_r2_key}" \
  dist/state/frozen-legacy-r2-final.json --endpoint-url "$r2_endpoint"
node scripts/updater/promotion-transaction.mjs validate-frozen-legacy \
  --current-policy dist/state/current-policy.json \
  --next-policy dist/state/policy-readback.json \
  --oss-legacy dist/state/frozen-legacy-oss-final.json \
  --r2-legacy dist/state/frozen-legacy-r2-final.json \
  --output dist/state/frozen-legacy-final-invariant.json
node scripts/updater/promotion-transaction.mjs poll-url \
  --url "$legacy_aliyun_url" \
  --expected dist/state/frozen-legacy-oss-final.json \
  --timeout-seconds 600 --interval-seconds 10 \
  --output dist/state/frozen-legacy-aliyun-cdn.json
node scripts/updater/promotion-transaction.mjs poll-url \
  --url "$legacy_r2_url" \
  --expected dist/state/frozen-legacy-r2-final.json \
  --timeout-seconds 600 --interval-seconds 10 \
  --output dist/state/frozen-legacy-r2-cdn.json

advance_journal committed router-policy-and-frozen-handoff-verified
trap - ERR INT TERM
delete_open_journal
