...
function approveRuntimeForExecution(config: BiyanTeamsConfig): BiyanTeamsConfig {
  if (config.runtime.approvedPlan) return config

  return approveBiyanTeamsPlan({
    ...config,
    runtime: {
      ...config.runtime,
...
describe('biyan teams runtime', () => {
  ...
  it('streams channel role output into the active assistant turn', async () => {
    ...
    const base = createDefaultBiyanTeamsConfig(model)
    const role = {
      id: 'streaming-worker',
  ...
  it('moves team role calls out of the current task delivery channel', async () => {
    ...
    const config = createDefaultBiyanTeamsConfig(model)
    let decisionCount = 0
  ...
  it('skips the scenario offer once the owner has declined it', async () => {
    const approved = approveRuntimeForExecution(
      createDefaultBiyanTeamsConfig({ provider: 'openai', id: 'gpt-5' })
    )
    const config = {
      ...approved,
  ...
  it('reuses existing roles and channels when a follow-up repeats configure_team', async () => {
    ...
    const firstRun = await runBiyanTeamsRuntime({
      config: approveRuntimeForExecution(config),
    ...
    const followUp = await runBiyanTeamsRuntime({
    ...
    const noOpFollowUp = await runBiyanTeamsRuntime({
...
美股调研容易触发财务合规风险，这个 demo 不使用 Market Research 团队。`,
  ...
  it('executes role calls only after the plan has been approved', async () => {
    ...
    const planned = await runBiyanTeamsRuntime({
      config: base,
    ...
    const approved = approveBiyanTeamsPlan(planned.config)
    const calledRoles: string[] = []
    let decisionCount = 0
  ...
  it('materializes approved plan role assignments before execution starts', async () => {
    ...
    const planned = normalizeBiyanTeamsConfig(
      {
        ...base,
        runtime: {
          ...base.runtime,
          ...
          planDraft: {
            ...
            version: 1,
            status: 'draft',
            goal: 'Design biyan.md generation.',
            ...
            roleAssignments: [
              {
                roleId: 'biyan_builder',
                name: 'Biyan Builder',
                assignment: 'Turn the approved biyan.md plan into generator logic.',
                model: 'openai/gpt-5',
                prompt: 'You build concrete biyan.md generator logic.',
              },
            ],
    ...
    const approved = approveBiyanTeamsPlan(planned)
    const calledRoles: string[] = []
    let decisionCount = 0
    ...
    expect(result.status).toBe('completed')
    expect(calledRoles).toEqual(['biyan_builder'])
  })
  ...
  it('rejects owner questions after a plan has already been approved', async () => {
    ...
    const approved = approveBiyanTeamsPlan(
      normalizeBiyanTeamsConfig(
        {
          ...base,
          runtime: {
  ...
  it('asks whether to keep role edits before applying a plan revision', () => {
    ...
    const edited = markBiyanTeamsRoleUserEdit(planned, 'orchestrator', [
      'prompt',
      'modelId',
    ])

    const revised = requestBiyanTeamsPlanRevision(
      edited,
