...
import {
  ...
  BIYAN_TEAMS_CHANNELS,
  BIYAN_TEAMS_MODES,
  BIYAN_TEAMS_ROLE_COLORS,
  BIYAN_TEAMS_TASK_TEMPLATES,
  createBiyanTeamsPlanDraft,
  normalizeBiyanTeamsId,
  type BiyanTeamsArtifactType,
  type BiyanTeamsArtifactUpdate,
  type BiyanTeamsChoiceOption,
  type BiyanTeamsChannelConfig,
  type BiyanTeamsChannelId,
  type BiyanTeamsConfig,
  type BiyanTeamsExecutionMode,
  type BiyanTeamsMilestone,
  type BiyanTeamsOrchestratorDecision,
  type BiyanTeamsPlanDraft,
  type BiyanTeamsRoleCallPlan,
  type BiyanTeamsRoleConfig,
  type BiyanTeamsRoleId,
  type BiyanTeamsScenarioId,
  type BiyanTeamsRuntime,
  type BiyanTeamsStructuredUpdates,
  type BiyanTeamsTaskStatus,
  type BiyanTeamsTaskUpdate,
  type BiyanTeamsTokenUsage,
...
type BiyanTeamsRoleWebSearchRequest = SearchDecision

type GenerateRoleText = (input: {
  role: BiyanTeamsRoleConfig
  prompt: string
  model?: ThreadModel
  webSearch?: BiyanTeamsRoleWebSearchRequest
...
type BiyanTeamsUserControl = {
  mentionedRoleIds: BiyanTeamsRoleId[]
  onlyRoleIds: BiyanTeamsRoleId[]
  shouldPause: boolean
  shouldContinue: boolean
  shouldConverge: boolean
}

type BiyanTeamsModelFamily =
  | 'chatgpt'
  | 'claude'
  | 'gemini'
  | 'grok'
...
type BiyanTeamsModelOption = {
  provider: string
  modelId: string
  label: string
  family: BiyanTeamsModelFamily
  nativeWebSearch: boolean
}

type BiyanTeamsUserWorkflowRoleSpec = {
  name: string
  key: string
  modelId?: string
}

type BiyanTeamsUserWorkflowSpec = {
  explicit: boolean
  roles: BiyanTeamsUserWorkflowRoleSpec[]
  channelLabel?: string
}

export type RunBiyanTeamsRuntimeOptions = {
...
export type BiyanTeamsRuntimeResult = {
  config: BiyanTeamsConfig
...
export type RunBiyanTeamsPrivateRoleChatOptions = {
  config: BiyanTeamsConfig
  roleId: BiyanTeamsRoleId
  userText: string
  abortSignal?: AbortSignal
...
export type BiyanTeamsPrivateRoleChatResult = {
  config: BiyanTeamsConfig
  finalResponse?: string
...
const usageFromUnknown = (value: unknown): BiyanTeamsTokenUsage | undefined => {
...
): { text: string; usage?: BiyanTeamsTokenUsage } =>
...
const addUsage = (
  current?: BiyanTeamsTokenUsage,
  incoming?: BiyanTeamsTokenUsage
): BiyanTeamsTokenUsage | undefined => {
  if (!current && !incoming) return undefined
  const usage: BiyanTeamsTokenUsage = {}
  const fields: Array<keyof BiyanTeamsTokenUsage> = [
    'promptTokens',
    'completionTokens',
    'totalTokens',
    'estimatedCostUsd',
...
const PERMISSION_RANK: Record<BiyanTeamsRoleConfig['permission'], number> = {
  read: 0,
  tools: 1,
  write: 2,
}

const TASK_STATUS_ALIASES: Record<string, BiyanTeamsTaskStatus> = {
  todo: 'todo',
  pending: 'todo',
  待确认: 'todo',
  待確認: 'todo',
...
const ARTIFACT_TYPE_ALIASES: Record<string, BiyanTeamsArtifactType> = {
  'decision': 'decision',
  '决策': 'decision',
  '決策': 'decision',
  'risk': 'risk',
...
const permissionMeets = (
  actual: BiyanTeamsRoleConfig['permission'],
  required?: BiyanTeamsRoleConfig['permission']
) => !required || PERMISSION_RANK[actual] >= PERMISSION_RANK[required]

const normalizeStatusAlias = (
  value: unknown
): BiyanTeamsTaskStatus | undefined => {
...
const normalizeArtifactTypeAlias = (
  value: unknown
): BiyanTeamsArtifactType | undefined => {
  if (typeof value !== 'string') return undefined
  return ARTIFACT_TYPE_ALIASES[value.trim().toLowerCase()]
}

function roleById(config: BiyanTeamsConfig, roleId: BiyanTeamsRoleId) {
  return config.roles.find((role) => role.id === roleId)
}

function channelById(config: BiyanTeamsConfig, channelId?: BiyanTeamsChannelId) {
  if (!channelId) return undefined
  return config.channels.find((channel) => channel.id === channelId)
}

function modeDescription(config: BiyanTeamsConfig) {
  const mode = BIYAN_TEAMS_MODES.find((item) => item.id === config.mode)
  return mode ? `${mode.label}: ${mode.description}` : config.mode
}

function modeOperationalGuidance(config: BiyanTeamsConfig) {
  switch (config.mode) {
    case 'relay':
...
type BiyanTeamsScenarioPlaybook = {
  id: BiyanTeamsScenarioId
  label: string
  taskTemplateId: BiyanTeamsConfig['taskTemplateId']
  mode: BiyanTeamsConfig['mode']
  flow: BiyanTeamsRoleId[]
  roles: Array<
    Omit<BiyanTeamsRoleConfig, 'color' | 'provider' | 'modelId' | 'enabled'>
  >
  channels: BiyanTeamsChannelConfig[]
  instructions: Record<BiyanTeamsRoleId, string>
}

const MARKET_RESEARCH_PLAYBOOK: BiyanTeamsScenarioPlaybook = {
  id: 'market_research',
...
function detectUserWorkflowSpec(userText: string): BiyanTeamsUserWorkflowSpec {
  const roles: BiyanTeamsUserWorkflowRoleSpec[] = []
...
function renderScenarioPlaybook(playbook?: BiyanTeamsScenarioPlaybook) {
...
function modelForRole(role: BiyanTeamsRoleConfig): ThreadModel | undefined {
...
const MODEL_FAMILY_LABELS: Record<BiyanTeamsModelFamily, string> = {
...
const MODEL_FAMILY_GUIDANCE: Record<BiyanTeamsModelFamily, string> = {
...
const familyFallbacks: Record<BiyanTeamsModelFamily, BiyanTeamsModelFamily[]> = {
...
function buildAvailableModelOptions(): BiyanTeamsModelOption[] {
  const state = useModelProvider.getState()
  const options: BiyanTeamsModelOption[] = []
  const seen = new Set<string>()
...
function renderModelAssignmentGuide(options: BiyanTeamsModelOption[]) {
  const familyGuide = (
    ['chatgpt', 'claude', 'gemini', 'grok'] as BiyanTeamsModelFamily[]
  )
    .map(
      (family) =>
...
- No configured alternate chat models found. Keep the current role model unless the owner configures more providers.`
  ...
  const groupedModels = (
    ['chatgpt', 'claude', 'gemini', 'grok', 'general'] as BiyanTeamsModelFamily[]
  )
    .map((family) => {
...
function resolveRoleModelConfig(role: BiyanTeamsRoleConfig): {
...
async function defaultGenerateDecisionText({
  ...
  const role: BiyanTeamsRoleConfig = {
...
function renderChannelRoster(config: BiyanTeamsConfig) {
...
function parseUserControl(
  userText: string,
  config: BiyanTeamsConfig
): BiyanTeamsUserControl {
  const mentionTokens = [...userText.matchAll(/@([^\s@，,。:：]+)/g)].map(
    (match) => match[1]
  )
  const roleMatches = new Set<BiyanTeamsRoleId>()

  for (const token of mentionTokens) {
    const normalized = normalizeBiyanTeamsId(token, '')
...
function renderUserControl(control: BiyanTeamsUserControl) {
  const parts = [
...
function hasOwnerSpecialistTeam(config: BiyanTeamsConfig) {
  return config.roles.some(
    (role) => role.enabled && role.id !== BIYAN_TEAMS_ORCHESTRATOR_ROLE_ID
  )
}

function hasReusableTeam(config: BiyanTeamsConfig) {
...
function renderWorkflowControlGuidance(
  workflowSpec?: BiyanTeamsUserWorkflowSpec
) {
...
function buildDecisionPrompt({
  ...
  userControl: BiyanTeamsUserControl
  modelOptions: BiyanTeamsModelOption[]
  scenarioPlaybook?: BiyanTeamsScenarioPlaybook
  ...
  const taskTemplate =
    BIYAN_TEAMS_TASK_TEMPLATES.find(
...
- #${BIYAN_TEAMS_TASK_CHANNEL_ID} / Current task is delivery-only: owner messages, choices, and final answer. Do not schedule specialist role calls or internal agent discussion there.
...
function webSearchDecisionForRoleCall({
  ...
  config: BiyanTeamsConfig
  role: BiyanTeamsRoleConfig
  call: BiyanTeamsRoleCallPlan
  userText: string
}): BiyanTeamsRoleWebSearchRequest {
  return decideBiyanTeamsRoleSearch({
...
function roleNativeSearchBlockedReason(role: BiyanTeamsRoleConfig): string | undefined {
  try {
    const resolved = resolveRoleModelConfig(role)
    if (
      canUseJingxingNativeWebSearch({
...
function isRoleId(value: unknown): value is BiyanTeamsRoleId {
  return typeof value === 'string' && normalizeBiyanTeamsId(value, '') === value
}

function isChannelId(value: unknown): value is BiyanTeamsChannelId {
  return typeof value === 'string' && normalizeBiyanTeamsId(value, '') === value
}

function isPermission(
  value: unknown
): value is BiyanTeamsRoleConfig['permission'] {
  return value === 'read' || value === 'tools' || value === 'write'
}

function isExecutionMode(value: unknown): value is BiyanTeamsExecutionMode {
...
function normalizeChoiceOptions(value: unknown): BiyanTeamsChoiceOption[] {
  if (!Array.isArray(value)) return []
  const seenOptionIds = new Set<string>()
  return value
    .map((item, index): BiyanTeamsChoiceOption | undefined => {
      if (!item || typeof item !== 'object') return undefined
      const raw = item as Partial<BiyanTeamsChoiceOption>
      ...
      const option: BiyanTeamsChoiceOption = {
        id,
        label,
      }
    ...
    .filter((item): item is BiyanTeamsChoiceOption => item !== undefined)
    .slice(0, 4)
}

function channelContainsRole(
  channel: BiyanTeamsChannelConfig,
  roleId: BiyanTeamsRoleId
) {
  return channel.roleIds.includes(roleId)
}

function findChannelForRole(config: BiyanTeamsConfig, roleId: BiyanTeamsRoleId) {
...
function normalizeCalls(
  value: unknown,
  config: BiyanTeamsConfig,
  options: { onlyRoleIds?: BiyanTeamsRoleId[] } = {}
): BiyanTeamsRoleCallPlan[] {
  if (!Array.isArray(value)) return []
  ...
  return value
    .map((item): BiyanTeamsRoleCallPlan | undefined => {
      if (!item || typeof item !== 'object') return undefined
      const raw = item as Partial<BiyanTeamsRoleCallPlan>
      ...
      if (!permissionMeets(role.permission, requiredPermission))
        return undefined
      const requestedChannelId = normalizeBiyanTeamsId(raw.channelId, '')
      let channelId =
      ...
      const call: BiyanTeamsRoleCallPlan = {
        roleId,
      ...
      if (Array.isArray(raw.dependsOn)) {
        const dependsOn = raw.dependsOn
          .map((id) => normalizeBiyanTeamsId(id, ''))
          .filter(
            (id): id is BiyanTeamsRoleId => isRoleId(id) && enabledIds.has(id)
          )
    ...
    .filter((item): item is BiyanTeamsRoleCallPlan => item !== undefined)
    .slice(0, 8)
}
...
function normalizeDecisionRoles(
  ...
  if (!Array.isArray(value)) return []
  const orchestrator = roleById(config, BIYAN_TEAMS_ORCHESTRATOR_ROLE_ID)
  const modelSource = orchestrator ?? config.roles[0]

  return value
    .map((item, index): BiyanTeamsRoleConfig | undefined => {
      if (!item || typeof item !== 'object') return undefined
      const raw = item as Partial<BiyanTeamsRoleConfig>
      const id = normalizeBiyanTeamsId(raw.id, '')
      if (!isRoleId(id) || id === BIYAN_TEAMS_ORCHESTRATOR_ROLE_ID) {
        return undefined
      }
    ...
    .filter((item): item is BiyanTeamsRoleConfig => item !== undefined)
    .slice(0, 8)
}

function mergeRoles(
  currentRoles: BiyanTeamsRoleConfig[],
  incomingRoles: BiyanTeamsRoleConfig[],
  archivedRoleIds?: ReadonlySet<BiyanTeamsRoleId>
) {
...
function archivedRoleIdSet(
  config: BiyanTeamsConfig
): ReadonlySet<BiyanTeamsRoleId> | undefined {
...
): BiyanTeamsChannelConfig[] {
  if (!Array.isArray(value)) return []
  const validRoleIds = new Set(roles.map((role) => role.id))

  return value
    .map((item): BiyanTeamsChannelConfig | undefined => {
      if (!item || typeof item !== 'object') return undefined
      const raw = item as Partial<BiyanTeamsChannelConfig>
      const id = normalizeBiyanTeamsId(raw.id, '')
      ...
      const roleIds = Array.isArray(raw.roleIds)
        ? raw.roleIds
            .map((roleId) => normalizeBiyanTeamsId(roleId, ''))
            .filter(
              (roleId): roleId is BiyanTeamsRoleId =>
      ...
      const fallbackRoleIds =
        ...
        validRoleIds.has(BIYAN_TEAMS_ORCHESTRATOR_ROLE_ID)
          ? [BIYAN_TEAMS_ORCHESTRATOR_ROLE_ID]
          : [])
    ...
    .filter((item): item is BiyanTeamsChannelConfig => item !== undefined)
    .slice(0, 8)
}

function mergeChannels(
  currentChannels: BiyanTeamsChannelConfig[],
  incomingChannels: BiyanTeamsChannelConfig[]
) {
...
function defaultDiscussionChannel(roleIds: BiyanTeamsRoleId[] = []) {
  ...
  return {
    ...
    roleIds,
  } satisfies BiyanTeamsChannelConfig
}
...
function normalizeChannelMembership(
  ...
  const validRoleIds = new Set(roles.map((role) => role.id))
  const taskDiscussionRoleIds = new Set<BiyanTeamsRoleId>()

  const normalizedChannels = channels.map((channel) => {
    const roleIds = channel.roleIds.filter((roleId) => validRoleIds.has(roleId))
    if (channel.id === BIYAN_TEAMS_TASK_CHANNEL_ID) {
      roleIds
        .filter((roleId) => roleId !== BIYAN_TEAMS_ORCHESTRATOR_ROLE_ID)
        .forEach((roleId) => taskDiscussionRoleIds.add(roleId))

      return {
        ...channel,
        roleIds: validRoleIds.has(BIYAN_TEAMS_ORCHESTRATOR_ROLE_ID)
          ? [BIYAN_TEAMS_ORCHESTRATOR_ROLE_ID]
          : [],
      }
    }
...
function applyScenarioPlaybook(
  config: BiyanTeamsConfig,
  playbook: BiyanTeamsScenarioPlaybook | undefined,
  modelOptions: BiyanTeamsModelOption[]
): BiyanTeamsConfig {
  ...
  const nextConfig: BiyanTeamsConfig = {
    ...config,
  ...
  return withBiyanTeamsRuntime(nextConfig, nextConfig.runtime)
}

function enforceTaskDeliveryChannel(config: BiyanTeamsConfig): BiyanTeamsConfig {
  const channels = normalizeChannelMembership(config.channels, config.roles)
  const channelSignature = (items: BiyanTeamsChannelConfig[]) =>
    JSON.stringify(
      items.map((channel) => ({
        id: channel.id,
...
function nextScenarioRequiredCall(
  config: BiyanTeamsConfig,
  runtime: BiyanTeamsRuntime,
  playbook?: BiyanTeamsScenarioPlaybook
): BiyanTeamsRoleCallPlan | undefined {
...
function findEnabledRoleByPattern(
  config: BiyanTeamsConfig,
  pattern: RegExp
): BiyanTeamsRoleConfig | undefined {
  return config.roles.find(
    (role) =>
      role.enabled &&
      role.id !== BIYAN_TEAMS_ORCHESTRATOR_ROLE_ID &&
...
function modeRequiredCallDecision(
  ...
  reason: string,
  instruction: string
): BiyanTeamsOrchestratorDecision | undefined {
...
function enforceModeBeforeStop(
  decision: BiyanTeamsOrchestratorDecision,
  config: BiyanTeamsConfig,
  runtime: BiyanTeamsRuntime
): BiyanTeamsOrchestratorDecision {
...
function normalizeTaskUpdates(
  value: unknown,
  config: BiyanTeamsConfig
): BiyanTeamsTaskUpdate[] {
  if (!Array.isArray(value)) return []
  const roleIds = new Set(config.roles.map((role) => role.id))
  const channelIds = new Set(config.channels.map((channel) => channel.id))

  return value
    .map((item): BiyanTeamsTaskUpdate | undefined => {
      if (!item || typeof item !== 'object') return undefined
      const raw = item as Partial<BiyanTeamsTaskUpdate>
      if (typeof raw.title !== 'string' || !raw.title.trim()) return undefined
      const status = normalizeStatusAlias(raw.status) ?? 'todo'
      const id = normalizeBiyanTeamsId(raw.id, '')
      const roleId = normalizeBiyanTeamsId(raw.roleId, '')
      const channelId = normalizeBiyanTeamsId(raw.channelId, '')
      const update: BiyanTeamsTaskUpdate = {
    ...
    .filter((item): item is BiyanTeamsTaskUpdate => item !== undefined)
    .slice(0, 12)
}
...
function normalizeArtifactUpdates(
  value: unknown,
  config: BiyanTeamsConfig
): BiyanTeamsArtifactUpdate[] {
  if (!Array.isArray(value)) return []
  const roleIds = new Set(config.roles.map((role) => role.id))
  const channelIds = new Set(config.channels.map((channel) => channel.id))

  return value
    .map((item): BiyanTeamsArtifactUpdate | undefined => {
      if (!item || typeof item !== 'object') return undefined
      const raw = item as Partial<BiyanTeamsArtifactUpdate>
      if (
      ...
      const roleId = normalizeBiyanTeamsId(raw.roleId, '')
      const channelId = normalizeBiyanTeamsId(raw.channelId, '')
      const update: BiyanTeamsArtifactUpdate = {
        type,
        title: trimText(raw.title, 120),
    ...
    .filter((item): item is BiyanTeamsArtifactUpdate => item !== undefined)
    .slice(0, 12)
}
...
function normalizeStructuredUpdates(
  value: unknown,
  config: BiyanTeamsConfig
): BiyanTeamsStructuredUpdates | undefined {
  if (!value || typeof value !== 'object') return undefined
  const raw = value as Partial<BiyanTeamsStructuredUpdates>
...
function taskIdFromUpdate(update: BiyanTeamsTaskUpdate) {
  return update.id ?? normalizeBiyanTeamsId(update.title, stableId('task'))
}

function artifactIdFromUpdate(update: BiyanTeamsArtifactUpdate) {
  return (
    update.id ??
    normalizeBiyanTeamsId(`${update.type}-${update.title}`, stableId('artifact'))
  )
}
...
function applyStructuredUpdates(
  ...
  source?: { roleId?: BiyanTeamsRoleId; channelId?: BiyanTeamsChannelId }
): BiyanTeamsRuntime {
  ...
  if (updates.tasks?.length) {
    ...
    nextRuntime = appendBiyanTeamsEvent(
      {
        ...nextRuntime,
...
function applyTeamConfiguration({
  ...
  config: BiyanTeamsConfig
  runtime: BiyanTeamsRuntime
  decision: Extract<BiyanTeamsOrchestratorDecision, { action: 'configure_team' }>
}) {
  const lockedToExistingRoster =
    isBiyanTeamsTeamLocked(config) &&
  ...
  const normalizedRuntime = withBiyanTeamsRuntime(nextConfig, runtime).runtime
  ...
  return {
    ...
    runtime: appendBiyanTeamsEvent(normalizedRuntime, {
...
function teamConfigurationDelta(
  ...
  const channels: BiyanTeamsChannelConfig[] = []
...
function workflowSpecForRole(
  workflowSpec: BiyanTeamsUserWorkflowSpec | undefined,
  role: Pick<BiyanTeamsRoleConfig, 'id' | 'name' | 'label'>
) {
...
function roleAssignmentAllowedByWorkflowSpec(
  workflowSpec: BiyanTeamsUserWorkflowSpec | undefined,
  assignment: { roleId: BiyanTeamsRoleId; name: string }
) {
...
function constrainPlanToWorkflowSpec(
  ...
  const allowedRoleIds = new Set<BiyanTeamsRoleId>([
    BIYAN_TEAMS_ORCHESTRATOR_ROLE_ID,
    ...config.roles
      .filter((role) =>
        lockToExistingRoles
  ...
  if (
    ...
    return {
      ...
      plan: constrainPlanToWorkflowSpec(
        config,
        decision.plan as BiyanTeamsPlanDraft,
        workflowSpec
      ),
    }
  }
    ...
    if (raw.action === 'propose_plan' || raw.action === 'revise_plan') {
      const rawPlan =
        raw.plan && typeof raw.plan === 'object'
          ...
          : raw
      const plan = createBiyanTeamsPlanDraft(
        rawPlan,
...
function describeDecisionEventTitle(
  action: BiyanTeamsOrchestratorDecision['action']
) {
  switch (action) {
    case 'configure_team':
...
function jsonFailureDecision(): BiyanTeamsOrchestratorDecision {
  return {
    action: 'ask_user',
    parseFallback: true,
    reason:
...
function updateRun(
  runtime: BiyanTeamsRuntime,
  patch: Partial<NonNullable<BiyanTeamsRuntime['run']>>
): BiyanTeamsRuntime {
...
function advanceRunRound(runtime: BiyanTeamsRuntime): BiyanTeamsRuntime {
  return updateRun(runtime, {
...
function groupCalls(
  decision: Extract<BiyanTeamsOrchestratorDecision, { action: 'call_roles' }>
) {
  if (decision.mode === 'parallel') return [decision.calls]
  if (decision.mode === 'serial') return decision.calls.map((call) => [call])

  const groups = new Map<number, BiyanTeamsRoleCallPlan[]>()
...
function parseRoleStructuredUpdates(
  ...
  const tasks: BiyanTeamsTaskUpdate[] = []
  const artifacts: BiyanTeamsArtifactUpdate[] = []
...
async function runRoleCall({
  ...
  generateRoleText: GenerateRoleText
  abortSignal?: AbortSignal
  notify?: (runtime: BiyanTeamsRuntime) => void
}): Promise<{
  runtime: BiyanTeamsRuntime
  output?: RoleOutput
  usage?: BiyanTeamsTokenUsage
  ...
  nextRuntime = appendBiyanTeamsEvent(nextRuntime, {
    type: 'role_called',
  ...
  try {
    ...
    const webSearch = blockedReason
      ...
      : requestedWebSearch
    trackBiyanEvent('web_search_decision_recorded', {
      source: 'biyan_teams_role_call',
      role_id: role.id,
...
function mergeBranchEvents(
  current: BiyanTeamsRuntime,
  branch: BiyanTeamsRuntime,
  branchBase: BiyanTeamsRuntime
) {
...
function planDraftFromExecutionDecision({
  ...
  const roleAssignments = (
    involvedRoles.length
      ? involvedRoles
      : virtualRoles.filter((role) => role.id === BIYAN_TEAMS_ORCHESTRATOR_ROLE_ID)
  ).map((role) => {
  ...
  return createBiyanTeamsPlanDraft(
    {
      goal,
      summary,
      scope: [
...
export async function runBiyanTeamsPrivateRoleChat({
  config,
  roleId,
  userText,
  abortSignal,
...
}: RunBiyanTeamsPrivateRoleChatOptions): Promise<BiyanTeamsPrivateRoleChatResult> {
  let workingConfig = withBiyanTeamsRuntime(config, config.runtime)
  ...
  const notify = (nextRuntime: BiyanTeamsRuntime) => {
    workingConfig = withBiyanTeamsRuntime(workingConfig, nextRuntime)
...
export async function runBiyanTeamsRuntime({
  config,
  userText,
  threadTitle,
  abortSignal,
...
}: RunBiyanTeamsRuntimeOptions): Promise<BiyanTeamsRuntimeResult> {
  ...
  const teamLocked = isBiyanTeamsTeamLocked(config)
  const workflowSpec =
  ...
  let workingConfig = enforceTaskDeliveryChannel(
    applyScenarioPlaybook(
      withBiyanTeamsRuntime(config, config.runtime),
      scenarioPlaybook,
      initialModelOptions
    )
  )
  ...
  try {
    while ((runtime.run?.currentRound ?? 0) < workingConfig.roundLimit) {
      ...
      if (decisionResolution.repairRequested) {
        runtime = appendBiyanTeamsEvent(runtime, {
      ...
      if (decision.action === 'clarify_user') {
        if ((runtime.clarificationCount ?? 0) >= 3) {
          const fallbackPlan = createBiyanTeamsPlanDraft(
            {
      ...
      if (
        ...
        const planDraft = createBiyanTeamsPlanDraft(
          {
            ...decision.plan,
            status: 'draft',
            revisionPrompt:
      ...
      if (decision.action === 'milestone') {
        const milestone: BiyanTeamsMilestone = {
          id: stableId('milestone'),
          title: decision.milestone,
          createdAt: nowIso(),
          sourceRoleId: BIYAN_TEAMS_ORCHESTRATOR_ROLE_ID,
        }
        runtime = appendBiyanTeamsEvent(
          {
            ...runtime,
